...
from bi_lstm_crf.app import predict
...
def __save_loss(losses, file_path):
    pd.DataFrame(
        data=losses, columns=["epoch", "batch", "train_loss", "val_loss"]
...
def train(args):
    ...
    preprocessor = Preprocessor(
        config_dir=args.corpus_dir, save_config_dir=args.model_dir, verbose=True
    )
    ...
    losses = (
        pd.read_csv(loss_path).values.tolist()
        if args.recovery and exists(loss_path)
        else []
    )
    ...
    for epoch in range(args.num_epoch):
        ...
        for bi, (xb, yb) in enumerate(bar):
            ...
            optimizer.step()
            bar.set_description(
                "{:2d}/{} loss: {:5.2f}, val_loss: {:5.2f}".format(
...
def main():
    ...
    parser.add_argument(
        "--corpus_dir",
        default="bi_lstm_crf/app/sample_corpus",
        type=str,
    ...
    parser.add_argument(
        "--num_epoch", type=int, default=20, help="number of epoch to train"
    )
    parser.add_argument("--lr", type=float, default=1e-3, help="learning rate")
    parser.add_argument(
        "--weight_decay", type=float, default=0.0, help="the L2 normalization parameter"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="the split for the validation dataset",
    )
    parser.add_argument(
        "--test_split",
        type=float,
        default=0.2,
        help="the split for the testing dataset",
    )
    parser.add_argument(
        "--recovery",
        action="store_true",
    ...
    parser.add_argument(
        "--num_rnn_layers", type=int, default=1, help="the number of RNN layers"
    )
    parser.add_argument(
        "--rnn_type", type=str, default="lstm", help='RNN type, choice: "lstm", "gru"'
    )
